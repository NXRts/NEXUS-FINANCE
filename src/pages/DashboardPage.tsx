  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Ringkasan Keuangan</h2>
          <p className="text-sm md:text-base text-slate-500 dark:text-slate-400 font-medium mt-1">Selamat datang kembali. Berikut ringkasan keuangan Anda hari ini.</p>
        </div>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          <div className="relative">
            <button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-primary text-white text-sm font-bold shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900 dark:bg-primary/20 p-6 rounded-2xl shadow-xl border border-slate-800 flex flex-col justify-between hover:shadow-2xl transition-all relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <h3 className="text-2xl xs:text-3xl sm:text-4xl font-extrabold text-white">{balance < 0 ? '-' : ''} {formatCurrency(balance)}</h3>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50 dark:bg-slate-800/50">
              <tr>
                <th className="px-4 md:px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Tanggal</th>
                <th className="px-4 md:px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Deskripsi</th>
                <th className="px-4 md:px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider hidden sm:table-cell">Kategori</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {recentTransactions.map((tx) => (
                <tr key={tx.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  <td className="px-4 md:px-6 py-5 text-sm font-medium text-slate-600 dark:text-slate-400 whitespace-nowrap">{tx.date}</td>
                  <td className="px-4 md:px-6 py-5">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-bold text-slate-900 dark:text-white truncate max-w-[100px] xs:max-w-[150px] md:max-w-[200px]">{tx.description}</span>
                    </div>
                  </td>
                  ...
                  <td className={cn("px-4 md:px-6 py-5 text-right font-bold whitespace-nowrap", tx.type === 'income' ? 'text-emerald-600 dark:text-emerald-400' : 'text-terracotta')}>
